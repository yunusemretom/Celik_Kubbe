...
bool joystick_init() {
    ...
    LOG_INFO("AP kuruldu: %s", JOYSTICK_WIFI_SSID);
    LOG_INFO("Kart adresi: %s", WiFi.softAPIP().toString().c_str());
    wifiOk = true;
    ...
    if (wifiOk) {
        LOG_INFO("STA IP: %s", WiFi.localIP().toString().c_str());
        if (MDNS.begin(JOYSTICK_MDNS_NAME)) {
            LOG_INFO("mDNS: %s.local", JOYSTICK_MDNS_NAME);
        }
    } else {
        LOG_WARN("WiFi baglanamadi, UDP yolu kapali");
    }
#endif

#if JOYSTICK_WIFI_MODE
    udp.begin(JOYSTICK_UDP_PORT);
    LOG_INFO("UDP portu: %d", JOYSTICK_UDP_PORT);
    return wifiOk;
#else
    LOG_INFO("WiFi devre disi");
    return false;
#endif
}
