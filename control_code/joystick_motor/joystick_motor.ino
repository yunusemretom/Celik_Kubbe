...
#define BAGLANTI_MODU 0

#define WIFI_SSID   "CelikKubbe"      // MODU 1: baglanilacak ag | MODU 2: kurulacak ag
#define WIFI_SIFRE  "parsTakimi2026"  // en az 8 karakter (AP modunda sart)
...
#define SERVO_PIN         10
#define SERVO_DURUS_US  1500    // notr pals. Servo bosta yavasca kayiyorsa
                                // buradan ince ayar yapin (1490/1510 gibi).
#define SERVO_TERS     false    // ileri/geri yonu ters geliyorsa true yapin
#define RT_ESIK        0.05f    // bunun altindaki RT = atis yok, servo durur

// --- Tek bir "tik": kol ileri gider, geri doner, bekler ---
// Surekli donus servosunda konum yoktur; ileri/geri hareket SUREYLE olculur.
// Ileri ve geri sureleri esit oldugu icin kol yaklasik ayni yere doner.
#define ATIS_STROK_MS     120   // ileri hareket suresi (ms) = kolun gidecegi yol
#define ATIS_GERI_CARPANI 1.0f  // geri sure = strok * bu. Kol her tikta bir miktar
                                // kayiyorsa 1.02 / 0.98 gibi ince ayarlayin.
#define ATIS_HIZ_SAPMA    400   // strok sirasinda notrden sapma (us) = kol hizi

// --- RT tetigi: saniyede kac tik atilacagi ---
#define ATIS_MIN_HZ      0.7f   // RT esigi yeni gecildiginde
#define ATIS_MAX_HZ      3.5f   // RT sonuna kadar cekildiginde. Ust sinir
                                // 1000/(strok+geri) ile sinirlidir; asilirsa
                                // kod otomatik kirpar (tik yarim kalmaz).
...
// ==================== ATIS SERVOSU ====================
float servoRT = 0.0f;             // PC'den gelen tetik degeri, 0..1 (0 = atis yok)
int   servoUs = SERVO_DURUS_US;   // servoya en son yazilan pals (teshis icin)

// Tek bir tikin asamalari. BOSTA disindaki her asamada servo hareket halindedir.
enum AtisFaz : uint8_t { ATIS_BOSTA, ATIS_ILERI, ATIS_GERI, ATIS_BEKLE };
AtisFaz       atisFaz = ATIS_BOSTA;
unsigned long atisFazBaslangic = 0;
unsigned long atisSayaci = 0;     // acilistan beri atilan tik sayisi (teshis)
...
// Geri donus suresi (ms).
inline unsigned long atisGeriSuresi() {
  return (unsigned long)(ATIS_STROK_MS * ATIS_GERI_CARPANI);
}

// RT'den saniyedeki tik sayisini bulur ve iki tik arasindaki bekleme suresini
// (ms) dondurur. Tik suresinden hizli istenirse bekleme 0'a kirpilir; boylece
// tikler ust uste binmez, sadece art arda gelir.
unsigned long atisBeklemeSuresi() {
  float oran = (servoRT - RT_ESIK) / (1.0f - RT_ESIK);
  if (oran < 0.0f) oran = 0.0f;
  if (oran > 1.0f) oran = 1.0f;

  float hz = ATIS_MIN_HZ + oran * (ATIS_MAX_HZ - ATIS_MIN_HZ);
  unsigned long periyot = (unsigned long)(1000.0f / hz);
  unsigned long tikSuresi = ATIS_STROK_MS + atisGeriSuresi();
  return periyot > tikSuresi ? periyot - tikSuresi : 0;
}

inline void atisFazaGec(AtisFaz faz, unsigned long simdi) {
  atisFaz = faz;
  atisFazBaslangic = simdi;
}

// Servoyu "tetige basip birakir gibi" surer: ileri - geri - bekle.
// RT yalnizca saniyede kac tik atilacagini belirler; hareketin kendisi hep ayni.
void servoGuncelle() {
  unsigned long simdi = millis();
  unsigned long gecen = simdi - atisFazBaslangic;

  int ileriPals = SERVO_TERS ? SERVO_DURUS_US - ATIS_HIZ_SAPMA
                             : SERVO_DURUS_US + ATIS_HIZ_SAPMA;
  int geriPals  = SERVO_TERS ? SERVO_DURUS_US + ATIS_HIZ_SAPMA
                             : SERVO_DURUS_US - ATIS_HIZ_SAPMA;

  switch (atisFaz) {
    case ATIS_BOSTA:
      servoDonanimaYaz(SERVO_DURUS_US);
      if (servoRT >= RT_ESIK) atisFazaGec(ATIS_ILERI, simdi);
      break;

    case ATIS_ILERI:
      servoDonanimaYaz(ileriPals);
      if (gecen >= ATIS_STROK_MS) atisFazaGec(ATIS_GERI, simdi);
      break;

    case ATIS_GERI:
      // Bu faz RT birakilsa bile tamamlanir: yarim kalirsa kol ileride takili
      // kalir ve bir sonraki tik yanlis yerden baslar.
      servoDonanimaYaz(geriPals);
      if (gecen >= atisGeriSuresi()) {
        atisSayaci++;
        atisFazaGec(servoRT >= RT_ESIK ? ATIS_BEKLE : ATIS_BOSTA, simdi);
      }
      break;
    ...
    case ATIS_BEKLE:
      servoDonanimaYaz(SERVO_DURUS_US);
      if (servoRT < RT_ESIK) {
        atisFazaGec(ATIS_BOSTA, simdi);
      } else if (gecen >= atisBeklemeSuresi()) {
        atisFazaGec(ATIS_ILERI, simdi);   // bir sonraki tik
      }
      break;
  }
}
...
void satirIsle(char *s) {
  ...
  *virgul = '\0';
  ...
  if (millis() - sonBasim > 200) {     // ~5 Hz; hatti bogmamak icin
    sonBasim = millis();
    char satir[80];
    snprintf(satir, sizeof(satir), "rx %.3f,%.3f,%.2f hiz %d,%d servo %d us tik %lu",
             eksen1, eksen2, rt, (int)m1.hiz, (int)m2.hiz, servoUs, atisSayaci);
    SERI.println(satir);
#if BAGLANTI_MODU
